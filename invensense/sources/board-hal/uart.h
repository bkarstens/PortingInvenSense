...
typedef enum uart_num {
	INV_UART_SENSOR_CTRL, /**< Just a fancier and hw independant name for UART0. It is used
	                                < to send command to/read data from sensors.
	                                < On samg55 it is mapped on CN6 USB port through FTDI.
	                                < UART0 has hw handshake capabilites. */
	INV_UART_LOG,         /**< Just a fancier and hw independant name for UART7. It is used
	                                < for SW traces.
 ...
 * initialize the uart peripheral. All fields need to be initilized in order to avoid 
 ...
 *  @param[in]  uart     UART peripheral
 *  @return     First available charcter in the RX FIFO
